...
type Handlers interface {
	BuybackStorage(log logrus.FieldLogger, c *configs.Config)
}
...
func (h *handler) BuybackStorage(log logrus.FieldLogger, c *configs.Config) {
	...
	for {
		...
		if err := c.DB().Model(&mr).Where("norek = ?", mr.Norek).First(&mr).Error; err != nil {
			if err == gorm.ErrRecordNotFound && mt.Type != os.Getenv("KAFKA_TOPIC") {
				mr.ID = mt.ID
